...
import { Form, Input, Upload, Button, Select, Space, Checkbox, InputNumber, Flex, message, Modal } from "antd";
...
function EditRecipe() {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [getAllCategories, getSingleRecipe, recipe_id]);
    ...
    <div>
      <Form
        ...
        initialValues={{
          ...
          recipe_instructions: recipe.recipe_instructions.map((instruction, index) => ({
      ...
      >
                ...
                {subFields.map((subField) => (
                  <Space key={subField.key}>
                    <Form.Item noStyle name={[subField.name, "ingredient"]}>
                      <TextArea placeholder="Write Ingredients" autoSize={{ minRows: 0, maxRows: 100 }} />
                    </Form.Item>
                    <CloseOutlined
                      onClick={() => {
                ...
                {subFields.map((subField) => (
                  <Space key={subField.key}>
                    <Form.Item noStyle name={[subField.name, "instruction"]}>
                      <TextArea placeholder="Write Your Instructions" autoSize={{ minRows: 0, maxRows: 100 }} />
                    </Form.Item>
                    <CloseOutlined
                      onClick={() => {
                        subOpt.remove(subField.name);
        ...
        <Flex style={{ marginTop: "20px", justifyContent: "center", gap: "20px" }}>
          ...
          {/* Delete Button */}
          <Form.Item wrapperCol={{ offset: 6, span: 14 }}>
            <Button onClick={() => handleRecipeDelete()} className="bg-primary text-white bold disable-hover">
              Delete
            </Button>
          </Form.Item>
